...
class AreaDao {
  ...
  Future<String> insertArea(
    ...
    final count = await (_db.select(
      _db.areas,
    )..where((t) => t.regionId.equals(regionId))).get().then((r) => r.length);
  ...
  Future<void> deleteArea(String id) async {
    final spots = await (_db.select(
      _db.spots,
    )..where((t) => t.areaId.equals(id))).get();
  ...
  Future<void> moveToRegion(String areaId, String newRegionId) async {
    final count =
        await (_db.select(_db.areas)
            ...
            .get()
            .then((r) => r.length);
    await (_db.update(_db.areas)..where((t) => t.id.equals(areaId))).write(
      AreasCompanion(regionId: Value(newRegionId), order: Value(count)),
    );
  }

  Future<void> copyToRegion(
    String areaId,
    String newRegionId,
    SpotDao spotDao,
  ) async {
    ...
    if (area == null) return;
    final newAreaId = await insertArea(
      area.name,
      area.type,
