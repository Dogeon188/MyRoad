...
class JsonExportService {
  ...
  Future<Map<String, dynamic>> exportRegion(String regionId) async {
    ...
    for (final area in areas) {
      ...
      for (final spot in spots) {
        final customInfos = await (_db.select(_db.spotCustomInfos)
              ..where((t) => t.spotId.equals(spot.id)))
            .get();
        final hours = await (_db.select(_db.spotOpeningHoursEntries)
              ..where((t) => t.spotId.equals(spot.id)))
            .get();
        ...
        spotsJson.add({
          ...
          'openingHours': hours
              .map((h) =>
                  {'day': h.day, 'open': h.openMinutes, 'close': h.closeMinutes})
              .toList(),
        });
      }
  ...
  Future<Map<String, dynamic>> exportTrip(String tripId) async {
    ...
    final days = await (_db.select(_db.itineraryDays)
        ...
        .get();
    final daysJson = await Future.wait(days.map((day) async {
