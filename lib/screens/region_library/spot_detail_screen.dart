...
class _SpotDetailScreenState extends ConsumerState<SpotDetailScreen> {
  ...
  Future<void> _loadSpot() async {
    ...
    final region = area != null
        ? await ref.read(regionDaoProvider).getById(area.regionId)
        : null;
            ...
            GestureDetector(
              onTap: () async {
                ...
                if (name != null && name.isNotEmpty) {
                  await ref
                      .read(spotDaoProvider)
                      .updateSpot(widget.spotId, name: name);
              ...
              onPressed: () {
                ...
                if (_spot!.googlePlaceId != null) {
                  uri = Uri.https('www.google.com', '/maps/search/', {
                    'api': '1',
                    'query': _spot!.name,
                    'query_place_id': _spot!.googlePlaceId!,
                  });
                } else {
                  uri = Uri.https('www.google.com', '/maps/search/', {
                    'api': '1',
                    'query': '${_spot!.lat},${_spot!.lng}',
                  });
                }
        ...
        children: [
              ...
              if (_spot!.type == 'hotel' && v.value != 'hotel') {
                ...
                if (refs.isNotEmpty) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(AppLocalizations.of(context)!.hotelInUse),
                    ),
                  );
          ...
          TextField(
            ...
            onChanged: (v) {
              ...
              setState(
                () => _spot = _spot!.copyWith(url: Value(v.isEmpty ? null : v)),
              );
            },
          ),
            ...
            children: [
              Expanded(
                child: TextField(
                  ...
                  onChanged: (v) => ref
                      .read(spotDaoProvider)
                      .updateSpot(widget.spotId, review: v),
                ),
              ),
              Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  IconButton(
                    ...
                    onPressed: () {
                      ...
                      ref
                          .read(spotDaoProvider)
                          .updateSpot(widget.spotId, rating: Value(r));
...
class _OpeningHoursSection extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_OpeningHoursSection> createState() =>
      _OpeningHoursSectionState();
}
      ...
      children: [
        ...
        FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            final minH = hours
                .map((h) => h.openMinutes ~/ 60)
                .reduce((a, b) => a < b ? a : b);
            final maxH = hours
                .map((h) => (h.closeMinutes / 60).ceil())
                .reduce((a, b) => a > b ? a : b);
                        ...
                        child: Column(
                          children: [
                            ...List.generate(hourCount, (i) {
                              ...
                              double openStart = mEnd.toDouble(),
                                  openEnd = mStart.toDouble();
                              for (final h in dayHours) {
                                final s = h.openMinutes
                                    .clamp(mStart, mEnd)
                                    .toDouble();
                                final e = h.closeMinutes
                                    .clamp(mStart, mEnd)
                                    .toDouble();
                                if (e > s) {
                                  openStart = openStart < s ? openStart : s;
                                  openEnd = openEnd > e ? openEnd : e;
                                }
                              }
                ...
                children: [
                  ...
                  Expanded(
                    child: TextButton(
                      ...
                      child: Text(
                        '${l10n.closeTime}: ${close.format(context)}',
                      ),
                    ),
                  ),
                ],
